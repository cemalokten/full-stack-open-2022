...
const App = () => {
  ...
  const deletePerson = async (id, name) => {
    try {
      ...
      setNewPerson({ data });
      setNotification({
        message: `The entry for ${name} has been deleted from the server`,
        type: "notification",
      });
    } catch (error) {
      setNotification({
        message: `The entry for ${name} has already been deleted from the server`,
        type: "error",
      });
      setTimeout(() => setNotification(null), 5000);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (handleDuplicate(person)) {
      if (window.confirm("The user already exists, update the phone number?")) {
        const { _id } = getUserByName(person);
        return updatePerson(person, _id);
      }
    } else {
